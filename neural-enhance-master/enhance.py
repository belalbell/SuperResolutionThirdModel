...
from lasagne.layers import InputLayer, ConcatLayer, ElemwiseSumLayer, batch_norm, rrelu
...
class Model(object):
    ...
    def make_res_block(self, name, input, units, filter_size=(3, 3), stride=(1, 1), pad=(1, 1), alpha=0.25):
        conv1 = ConvLayer(input, units, filter_size, stride=stride, pad=pad, nonlinearity=None)
        relu = rrelu(conv1)
        conv2 = ConvLayer(input, units, filter_size, stride=stride, pad=pad, nonlinearity=None)
    ...
    def make_decriminator_relu_block(self, name, input, units, filter_size=(3, 3), stride=(1, 1), pad=(1, 1),
                                     alpha=0.1):
        conv = ConvLayer(input, units, filter_size, stride=stride, pad=pad, nonlinearity=None)
        relu = rrelu(batch_norm(conv))

        self.network[name + 'x1'] = conv
        self.network[name + '#1'] = relu

        return relu

    def make_decriminator_tanh_block(self, name, input, units, filter_size=(3, 3), stride=(1, 1), pad=(1, 1)):
        conv = ConvLayer(input, units, filter_size, stride=stride, pad=pad, nonlinearity=None)
        tanh = lasagne.nonlinearities.tanh(batch_norm(conv))

        self.network[name + 'x1'] = conv
        self.network[name + '#1'] = tanh

        return tanh

    def make_decriminator_selu_block(self, name, input, units, filter_size=(3, 3), stride=(1, 1), pad=(1, 1),
                                     alpha=0.2):
        conv = ConvLayer(input, units, filter_size, stride=stride, pad=pad, nonlinearity=None)
        selu = lasagne.nonlinearities.selu(batch_norm(conv))

        self.network[name + 'x1'] = conv
        self.network[name + '#1'] = selu
    ...
    def setup_discriminator(self):
        ...
        self.make_decriminator_relu_block('disc1', self.network['conv1_2'], 64, filter_size=(3, 3), stride=(1, 1),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc2', self.last_layer(), 64, filter_size=(4, 4), stride=(2, 2),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc3', self.last_layer(), 128, filter_size=(3, 3), stride=(1, 1),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc4', self.last_layer(), 128, filter_size=(4, 4), stride=(2, 2),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc5', self.last_layer(), 256, filter_size=(3, 3), stride=(1, 1),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc6', self.last_layer(), 256, filter_size=(4, 4), stride=(2, 2),
                                          pad=(1, 1))
        self.make_decriminator_relu_block('disc7', self.last_layer(), 512, filter_size=(4, 4), stride=(2, 2),
                                          pad=(1, 1))

        self.network['disc'] = ConvLayer(self.last_layer(), 1, filter_size=(1, 1),
